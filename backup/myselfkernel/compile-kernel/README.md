...
| -n | CustomName | Set the kernel custom signature. The default value is `` and the generated kernel is `5.10.125`. Do not include spaces when setting a custom signature. |
...
- `sudo ./recompile -d -k 5.10.125 -n `: Use the default configuration, and use the `-n` parameter to set the kernel custom signature.
...
- `sudo ./recompile -d -k 5.10.125_5.15.50 -a true -n  -r kernel.org`: Use the default configuration, and set through multiple parameters.
